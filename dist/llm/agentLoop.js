...
exports.buildCoachingPrompt = buildCoachingPrompt;
exports.validateUnrelatedClaim = validateUnrelatedClaim;
exports.runAgentLoop = runAgentLoop;
const openaiClient_js_1 = require("./openaiClient.js");
const parseVerificationError_js_1 = require("../core/parseVerificationError.js");
...
function looksLikeCommandFailure(output) {
    ...
    return (/SyntaxError|ReferenceError|TypeError|RangeError/i.test(t) ||
...
function normalizePatchedPath(filePath) {
    return String(filePath || "").replace(/\\/g, "/").trim();
}
const SYNTAX_BROKEN_POST_WRITE_COACHING_PROMPT = `Your last apply_patch was applied but it BROKE the file's syntax (parse error reported). The file has been reverted to its pre-patch state.\n\n` +
...
const PRE_EXISTING_BROKEN_COACHING_PROMPT = `The file you tried to patch is ALREADY syntactically broken at the reported line - this breakage existed BEFORE your patch. Your patch was rejected because patching a syntactically invalid file is unsafe.\n\n` +
...
function extractSemanticSmellName(errorPreview) {
    const match = String(errorPreview || "").match(/smell detected was:\s*([a-z_]+)/i);
    return match?.[1]?.toLowerCase() ?? "unknown";
}
function getSemanticSmellSpecificGuidance(smellName) {
    switch (smellName) {
...
function classifyFailure(toolName, output, error) {
    ...
    if (toolName === "apply_patch") {
        ...
        if (/find content not found|find_not_found/i.test(text))
            return "apply_patch_find_not_found";
...
function buildCoachingPrompt(trigger, errorPreview, _recentToolCalls) {
    switch (trigger) {
        ...
        case "apply_patch_multiple_matches":
            return (`Your FIND block matched multiple locations. apply_patch refuses ambiguous edits.\n` +
                `Solutions, pick one:\n` +
        ...
        case "apply_patch_semantic_smell": {
            ...
            return (`Your last apply_patch was applied but it produced semantically broken output. The file has been reverted.\n\n` +
        ...
        case "apply_patch_syntax_broken_post_write":
            ...
            return (`Your patch was applied but produced invalid syntax â€” the file was reverted to its pre-patch state.\n` +
        ...
        case "apply_patch_pre_existing_broken":
            ...
            return (`The target file was already syntactically broken BEFORE your patch â€” this is not your fault but you must repair it.\n` +
        ...
        case "apply_patch_scope_not_found":
            return (`Your apply_patch failed because the scope symbol you specified does not exist in the file. ` +
                `Common causes:\n` +
        ...
        case "tool_command_spawn_failure":
            return (`The shell command failed to spawn. On Windows, common causes:\n` +
        ...
        case "tool_path_enoent":
            return (`A file path failed with ENOENT. Most common cause in Zone: passing an absolute path got concatenated to the cwd.\n` +
                `Solutions:\n` +
        ...
        case "test_failed":
            return (`Tests reported failure or warnings. Determine: is the failure RELATED to your change, ` +
        ...
        case "apply_patch_replace_shorter_than_find":
            return (`Your REPLACE block has FEWER lines than your FIND block, which apply_patch rejects to protect against accidental deletion.\n` +
                `Rules:\n` +
        ...
        case "apply_patch_find_block_empty":
            return (`Your FIND block is empty â€” apply_patch requires at least one line in FIND to locate the insertion point.\n` +
        ...
        case "unknown":
        default:
            return (`Something went wrong but the failure mode wasn't recognized automatically.\n` +
...
function extractFunctionCallItems(response) {
...
function inferVerificationFromLog(log) {
    const patchApplied = log.some((e) => (e.tool === "apply_patch" || e.tool === "write_file") &&
    ...
    const hasInfraError = log.some((e) => e.tool === "run_command" &&
        /spawn.*enoent|enoent.*cmd\.exe|missing script|command not found|cannot find/i.test(String(e.result || "")));
    const testsRan = log.some((e) => e.tool === "run_command" && /\bpassed\b|\b\d+ pass/i.test(String(e.result || "")));
    if (patchApplied && testsRan)
        return "tests_passed";
    if (patchApplied && hasInfraError)
        return "tests_inconclusive";
    if (!patchApplied)
        return "tests_failed_by_patch";
    return "no_verification_attempted";
}
function validateUnrelatedClaim(input) {
    const anyRunCommand = input.log.some((entry) => entry.tool === "run_command");
    const looksLikePassingRunCommand = (output) => {
        const text = String(output || "");
        return (/\ball tests passed\b/i.test(text) ||
    ...
    const failingRunCommand = [...input.log]
        .reverse()
        .find((entry) => isRunCommandFailure(entry) &&
        !looksLikePassingRunCommand(String(entry.result || "")));
    if (!anyRunCommand) {
        return {
            accept: false,
            demoteTo: "tests_inconclusive",
            reason: "no run_command in log â€” agent claimed test failure without ever running tests",
        };
    }
    if (!failingRunCommand) {
        return {
            accept: true,
            reason: "run_command(s) executed but none look like failure â€” accepting agent classification",
        };
    }
    const verificationError = (0, parseVerificationError_js_1.parseVerificationError)(String(failingRunCommand.result || ""), input.patchedFilePaths);
    ...
    const failingFileIsPatched = !!failingFile &&
        normalizedPatched.some((patchedFilePath) => patchedFilePath === failingFile || failingFile.endsWith(patchedFilePath));
    ...
    if (failingFileIsPatched) {
        return {
            ...
            reason: "failing file is in patchedFilePaths â€” agent's unrelated claim rejected",
        };
    }
    if (failingFile) {
        return {
    ...
    return {
        accept: false,
        demoteTo: "tests_inconclusive",
        reason: "cannot verify unrelated claim â€” no failing file extracted or evidence ambiguous",
    };
}
async function runAgentLoop(input) {
    ...
    console.log("[zone-agent-loop-entry]", JSON.stringify({
        ...
        maxIterations,
        toolsAvailable: toolDefinitions_js_1.ZONE_TOOLS.map((t) => t.name ?? "unknown"),
        hasRunId: !!(input.runId && input.runId.trim()),
    }));
    const fw = input.framework;
    const systemContent = `You are Zone, an AI code agent${fw?.framework ? ` working on a ${fw.framework} project` : ""}.\n\n` +
        (fw
            ? `Project info:\n` +
    ...
    for (let iter = 0; iter < maxIterations; iter += 1) {
        ...
        const response = await client.responses.create({
            model: (0, openaiClient_js_1.getModelName)("high"),
            input: responseInput,
            tools: toolDefinitions_js_1.ZONE_TOOLS,
            tool_choice: "auto",
        });
        ...
        if (toolCalls.length > 0) {
            ...
            for (const call of toolCalls) {
                ...
                console.log("[zone-agent-tool-call]", JSON.stringify({
                    ...
                    patchPreview: typeof parsedArgs.patch === "string"
                        ? parsedArgs.patch.slice(0, 400)
                        : null,
                    contentLength: typeof parsedArgs.content === "string"
                        ? parsedArgs.content.length
                        : null,
                    command: parsedArgs.command ?? null,
                }));
                const rid = String(input.runId || "").trim();
                const result = await (0, toolExecutor_js_1.executeTool)(name, parsedArgs, input.repoPath, input.onProgress, {
                    runId: rid || undefined,
                ...
                const toolFailed = (name === "run_command" && looksLikeCommandFailure(result.output)) ||
            ...
            if (failureDetected && selfCorrectionAttempts < MAX_SELF_CORRECTION_ATTEMPTS) {
                ...
                input.onProgress?.(`[agent_loop] Failure detected (${routedTrigger}) â€” self-correction attempt ${selfCorrectionAttempts}/${MAX_SELF_CORRECTION_ATTEMPTS}`);
                responseInput.push({
                    role: "user",
                    type: "message",
                    content: `[Zone coaching â€” attempt ${selfCorrectionAttempts} of ${MAX_SELF_CORRECTION_ATTEMPTS}]\n` +
                        coachingText +
        ...
        const extracted = (0, openaiClient_js_1.extractResponsesApiOutputText)(response);
        if (extracted.ok && extracted.text.trim()) {
            ...
            let verificationReason = validReasons.includes(vrRaw)
                ? vrRaw
            ...
            const patchValidatedByAgent = verificationReason === 'tests_passed' ||
    ...
    let finalVerificationReason = inferVerificationFromLog(toolCallLog);
    let inferredFrom = "heuristic";
    ...
    try {
        ...
        const ae = (0, openaiClient_js_1.extractResponsesApiOutputText)(assessmentResponse);
    ...
    const patchValidatedByAgent = finalVerificationReason === "tests_passed" ||
