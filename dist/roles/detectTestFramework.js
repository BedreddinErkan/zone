...
function detectTestFramework(files) {
    ...
    if (hasFile(files, "playwright.config.ts")) {
        ...
        const hasSpecTs = files.some((f) => f.path.endsWith(".spec.ts") || f.path.endsWith(".test.ts"));
        if (hasSpecTs)
            evidence.push(".spec.ts test files found");
        return {
    ...
    // Cypress
    if (hasFile(files, "cypress.config.ts") ||
    ...
    if (hasFile(files, "pom.xml") &&
        ...
        if (hasSelenium)
            evidence.push("Selenium page objects detected");
        return {
    ...
    if (hasExtension(files, ".py")) {
        const hasSelenium = files.some((f) => f.path.includes("selenium") || f.path.includes("webdriver"));
        if (hasSelenium) {
