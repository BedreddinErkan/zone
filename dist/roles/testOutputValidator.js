...
exports.validateTestOutput = validateTestOutput;
// ─── Feature Validator ────────────────────────────────────────────────────────
function validateFeatureFile(content) {
    const issues = [];
...
function extractMethodCalls(javaContent) {
...
function extractMethodDefinitions(javaContent) {
...
function validateStepDefinitions(content, pageObjectContents) {
    const issues = [];
    // TODO/stub/placeholder kontrolü
    const todoPatterns = ["// TODO", "// FIXME", "// STUB", "// implement"];
    ...
    if (pageObjectContents.length > 0) {
        const allPageObjectMethods = new Set();
...
function validatePlaywrightTest(content) {
    const issues = [];
    ...
    for (const line of lines) {
        ...
        if (/(?:^|[^a-zA-Z0-9_])(?:page|[\w$.()[\]]+)\.(?:click|fill)\s*\(/.test(trimmed) &&
...
function extractPythonImportedNames(content) {
    ...
    for (const line of lines) {
        ...
        if (importMatch) {
            ...
            for (const name of names) {
                ...
                if (baseName)
                    importedNames.add(baseName.trim());
            }
        }
        ...
        if (fromImportMatch) {
            ...
            for (const name of names) {
                ...
                if (importedName)
                    importedNames.add(importedName.trim());
            }
        }
    }
    return importedNames;
}
function extractPytestFunctions(content) {
    const functions = [];
    const lines = content.split("\n");
...
function validatePytestTest(content) {
    const issues = [];
    const lowerContent = content.toLowerCase();
    ...
    for (const { name: testName, params, body } of testFunctions) {
        ...
        while ((variableMatch = variableUseRegex.exec(body)) !== null) {
            ...
            if (!availableNames.has(variableName) &&
                variableName !== "pytest" &&
...
function validateSqlOutput(content, dialect) {
    const issues = [];
    const upperContent = content.toUpperCase();
    if (/\bDROP\s+TABLE\b/.test(upperContent) ||
    ...
    const createTableBlockRegex = /CREATE\s+TABLE(?:\s+IF\s+NOT\s+EXISTS)?[\s\S]*?\(([\s\S]*?)\)/gi;
    let blockMatch;
...
// ─── Cypress Validator ────────────────────────────────────────────────────────
function validateCypressTest(content) {
    const issues = [];
...
function validateSeleniumJavaTest(content) {
    const issues = [];
    if (!content.includes("package ")) {
        issues.push({
    ...
    if (!content.includes("assertTrue") &&
...
// ─── TestNG Validator ─────────────────────────────────────────────────────────
function validateTestNGTest(content) {
    const issues = [];
...
function validateComplexity(content, framework, complexity) {
    const issues = [];
    const lowerContent = content.toLowerCase();
    if (complexity === "data_driven") {
        if ((framework === "pytest" || framework === "selenium_python") &&
        ...
        if ((framework === "playwright_ts" || framework === "playwright_js") &&
...
// ─── Main Validator ───────────────────────────────────────────────────────────
function validateTestOutput(input) {
    const issues = [];
    if (input.featureContent) {
        issues.push(...validateFeatureFile(input.featureContent));
    }
    if (input.stepDefinitionContent) {
        issues.push(...validateStepDefinitions(input.stepDefinitionContent, input.pageObjectContents ?? []));
    }
    if ((input.framework === "playwright_ts" || input.framework === "playwright_js") &&
        (input.testFileContent || input.featureContent)) {
        issues.push(...validatePlaywrightTest(input.testFileContent ?? input.featureContent ?? ""));
    }
    if (input.framework === "cypress" && input.testFileContent) {
        issues.push(...validateCypressTest(input.testFileContent));
    }
    if ((input.framework === "selenium_java" || input.framework === "testng") &&
    ...
    if ((input.framework === "pytest" || input.framework === "selenium_python") &&
        input.testFileContent) {
    ...
    if (input.complexityHint) {
        const complexityContent = input.testFileContent ??
            input.featureContent ??
            input.stepDefinitionContent ??
            "";
        issues.push(...validateComplexity(complexityContent, input.framework, input.complexityHint));
    }
...
//# sourceMappingURL=testOutputValidator.js.map