...
exports.buildDataAnalystContext = buildDataAnalystContext;
...
function buildSlug(task) {
    return buildIntentTokens(task).join("_");
}
function detectSqlFiles(files) {
...
function detectMigrationFiles(files, migrationFormat) {
    if (!Array.isArray(files))
        return [];
    ...
    if (migrationFormat === "alembic") {
        return files
            .filter((file) => file.path.includes("alembic/versions/") || file.path === "alembic.ini")
            .sort((a, b) => a.path.localeCompare(b.path));
    }
    if (migrationFormat === "liquibase") {
        return files
            .filter((file) => file.path.toLowerCase().includes("liquibase") ||
...
function buildOutputRules(dialect, existingTables) {
    const common = [
        ...
        `Never invent table names that conflict with existing tables: ${existingTables.length > 0 ? existingTables.join(", ") : "none"}`,
    ];
...
function getNextFlywayVersion(migrationFiles) {
    let maxVersion = 0;
...
function buildOutputPath(task, schema, migrationFiles) {
...
function buildDataAnalystContext(task, schema, files) {
...
//# sourceMappingURL=dataAnalystContext.js.map