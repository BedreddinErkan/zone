...
exports.resolveRunCommandCwd = resolveRunCommandCwd;
...
const astSymbolLocator_js_1 = require("../ast/astSymbolLocator.js");
const astSyntaxValidator_js_1 = require("../ast/astSyntaxValidator.js");
...
(function verifyToolDispatch() {
    ...
    try {
        ...
        defs = require("./toolDefinitions.js")
            .ZONE_TOOLS;
    }
    catch {
    ...
    for (const tool of defs) {
        if (!DISPATCHED_TOOLS.has(tool.name)) {
            throw new Error(`FATAL: Tool '${tool.name}' is defined in toolDefinitions but has no executor dispatch. ` +
...
function visibleEolPreview(s, maxChars = 200) {
    return String(s || "")
        .slice(0, maxChars)
        .replace(/\r/g, "<CR>")
        .replace(/\n/g, "<LF>\n");
}
function hasTrailingNewline(s) {
    return /\r?\n$/.test(String(s || ""));
}
function resolveRunCommandCwd(rawCwd, repoPath) {
    let candidate;
    if (rawCwd === null ||
        rawCwd === undefined ||
        ...
        candidate = node_path_1.default.isAbsolute(rawCwd)
            ? node_path_1.default.normalize(rawCwd)
            : node_path_1.default.resolve(repoPath, rawCwd);
    }
    else {
        return {
            ok: false,
    ...
    try {
        ...
        return {
            ok: false,
            error: `cwd does not exist: ${candidate}. If you intended a path inside the repo, ` +
 ...
 */
function countCrlfsBefore(s, offset) {
    let count = 0;
...
async function executeTool(toolName, toolArgs, repoPath, onProgress, input) {
    const args = (toolArgs ?? {});
    try {
        ...
        if (toolName === "read_file") {
            ...
            const abs = node_path_1.default.join(repoPath, filePath);
            const content = node_fs_1.default.readFileSync(abs, "utf8");
        ...
        if (toolName === "apply_patch") {
            ...
            let original;
            try {
                original = node_fs_1.default.readFileSync(abs, "utf8");
            }
            catch {
                return {
                    success: false,
            ...
            {
                const preflightValidation = (0, astSyntaxValidator_js_1.validateSyntax)(originalWithoutBom, abs);
                const scopeRequested = args.scope != null;
                if (!preflightValidation.ok && preflightValidation.reason === "parse_error") {
                    console.log("[zone-apply-patch-preflight]", JSON.stringify({
                        filePath,
                        preflightOk: false,
                        reason: "parse_error",
                    ...
                    const baseMsg = `The file is already syntactically broken at ${locationHint}: ${preflightValidation.errorMessage ?? "parse error"}.\n` +
            ...
            console.log("[zone-apply-patch-file-preview]", JSON.stringify({
                filePath,
            ...
            while (remaining.includes(FIND_MARKER)) {
                ...
                const replaceContent = nextFindIdx === -1 ? afterReplace : afterReplace.slice(0, nextFindIdx);
                blocks.push({
            ...
            if (blocks.length === 0) {
                return {
                    success: false,
                    output: "No valid --- FIND --- / --- REPLACE --- blocks found in patch. " +
                        "Format: --- FIND ---\n<exact code>\n--- REPLACE ---\n<code with additions>",
                };
            }
            let currentNormalized = originalWithoutBom.replace(/\r\n/g, "\n");
            const scopeArg = args.scope ?? null;
            let activeScopeInfo = null;
            if (scopeArg !== null) {
                const kindRaw = scopeArg.symbolKind;
                const descriptor = {
                    name: scopeArg.symbolName,
                    kind: (kindRaw != null && kindRaw !== "" ? kindRaw : "any"),
                    className: scopeArg.className ?? undefined,
                };
                const locateResult = (0, astSymbolLocator_js_1.locateSymbol)(originalWithoutBom, abs, descriptor);
                if (!locateResult.ok && locateResult.reason === "unsupported_extension") {
                    console.log("[zone-apply-patch-scope]", JSON.stringify({
                        filePath,
                    ...
                    return {
                        success: false,
                        output: `Could not parse file to apply scope-bounded patch: ` +
                    ...
                    return {
                        success: false,
                        output: `Scope symbol '${scopeArg.symbolName}' (kind: ${descriptor.kind}) not found in ${filePath}. ` +
                    ...
                    return {
                        success: false,
                        output: `Scope symbol '${scopeArg.symbolName}' has ${locateResult.matches.length} occurrences ` +
                    ...
                    const normStart = match.startChar - countCrlfsBefore(originalWithoutBom, match.startChar);
                    const normEnd = match.endChar - countCrlfsBefore(originalWithoutBom, match.endChar);
                    activeScopeInfo = {
            ...
            for (let bi = 0; bi < blocks.length; bi += 1) {
                ...
                console.log("[zone-apply-patch-find-preview]", JSON.stringify({
                    filePath,
                    block: bi + 1,
                ...
                if (findHadCrOnly || replaceHadCrOnly) {
                    console.log("[zone-apply-patch-eol-warn]", JSON.stringify({
                        filePath,
                        block: bi + 1,
                ...
                const searchTarget = activeScopeInfo !== null
                    ? currentNormalized.slice(currentScopeStart, currentScopeEnd)
                    : currentNormalized;
                const normalizedOccurrences = normalizedFind.length === 0
                    ? 0
                    : countOccurrences(searchTarget, normalizedFind);
                console.log("[zone-apply-patch-eol]", JSON.stringify({
                    filePath,
                    originalEol,
                    fileHadBOM,
                ...
                if (normalizedFind.length === 0) {
                    console.log("[zone-apply-patch-debug]", JSON.stringify({
                        ...diagBase,
                        rejected: true,
                ...
                if (normalizedReplace.length === 0) {
                    ...
                    return {
                        success: false,
                        output: `Block ${bi + 1}: REPLACE block is empty. ` +
                ...
                if (normalizedOccurrences === 0) {
                    ...
                    return {
                        success: false,
                        output: `Block ${bi + 1}: FIND content not found in file` +
                ...
                if (normalizedOccurrences > 1) {
                    ...
                    return {
                        success: false,
                        output: `Block ${bi + 1}: FIND content matches ${normalizedOccurrences} locations` +
                            (activeScopeInfo !== null
                ...
                if (!allowShrink && replaceLineCount < findLineCount) {
                    ...
                    return {
                        success: false,
                        output: `Block ${bi + 1}: REPLACE has ${replaceLineCount} lines but FIND has ${findLineCount} lines. ` +
            ...
            let outputContent = reEncodedTo === "crlf"
            ...
            node_fs_1.default.writeFileSync(abs, outputContent, "utf8");
            // ─── Post-write syntax validation ────────────────────────────────────────
            const validation = (0, astSyntaxValidator_js_1.validateSyntax)(outputContent, abs);
            const syntaxBroken = !validation.ok && validation.reason === "parse_error";
            const smellValidation = validation.ok && validation.reason !== "unsupported_extension"
                ? (0, astSyntaxValidator_js_1.checkSemanticSmells)(outputContent, abs, validation.ast)
                : { ok: true };
            const semanticSmellDetected = !smellValidation.ok;
            console.log("[zone-apply-patch-syntax-validation]", JSON.stringify({
                filePath,
            ...
            if (syntaxBroken) {
                ...
                node_fs_1.default.writeFileSync(abs, original, "utf8");
                return {
                    success: false,
            ...
            if (semanticSmellDetected) {
                ...
                return {
                    success: false,
                    output: `Patch applied but produced semantically broken output. ` +
        ...
        if (toolName === "write_file") {
            ...
            let originalContent = "";
            try {
                originalContent = node_fs_1.default.readFileSync(abs, "utf8");
            ...
            console.log(`[zone-agent-write-debug] ${JSON.stringify({
                tool: "write_file",
                filePath,
                originalSize,
                newSize,
            ...
            if (blocked) {
                return {
                    success: false,
                    output: `write_file blocked: new content (${newSize} chars) is only ` +
            ...
            node_fs_1.default.writeFileSync(abs, content, "utf8");
            const validation = (0, astSyntaxValidator_js_1.validateSyntax)(content, abs);
            const syntaxBroken = !validation.ok && validation.reason === "parse_error";
            const smellValidation = validation.ok && validation.reason !== "unsupported_extension"
                ? (0, astSyntaxValidator_js_1.checkSemanticSmells)(content, abs, validation.ast)
                : { ok: true };
            const semanticSmellDetected = !smellValidation.ok;
            console.log("[zone-write-file-validation]", JSON.stringify({
                filePath,
            ...
            if (syntaxBroken || semanticSmellDetected) {
                if (fileExists) {
                    node_fs_1.default.writeFileSync(abs, originalContent, "utf8");
                }
                else {
                    try {
        ...
        if (toolName === "search_in_files") {
            ...
            const matches = [];
            const matchCountsByFile = new Map();
            ...
            for (const rel of files) {
                ...
                let text = "";
                try {
                    text = node_fs_1.default.readFileSync(node_path_1.default.join(repoPath, rel), "utf8");
                }
                catch {
                    continue;
                }
            ...
            if (capReached) {
                summaryLines.push(`WARNING: CAP REACHED at ${maxMatches} matches - there may be more results. ` +
        ...
        if (toolName === "find_references") {
            ...
            const allFiles = await (0, fast_glob_1.default)("**/*.{ts,tsx,js,jsx,mjs,cjs}", {
                cwd: repoPath,
                dot: false,
                onlyFiles: true,
                unique: true,
            ...
            if (!sourceNode) {
                return {
                    success: false,
                    output: `Source file not found in dependency graph: ${sourceFile}. ` +
