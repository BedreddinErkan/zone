...
function isSafeReplaceOperation(operation) {
...
function canConvertGeneratedPlanToPatchPlan(plan) {
    ...
    if (!isSafeReplaceOperation(operation)) {
        return fail("UNSUPPORTED_OPERATION", `Generated patch operation "${operation.type}" is not supported for PatchPlan conversion.`);
    }
    if (!operation.filePath.trim()) {
        return fail("MISSING_REQUIRED_FIELDS", "Generated patch operation requires a non-empty filePath.");
    }
    if (!operation.find.trim()) {
        return fail("MISSING_REQUIRED_FIELDS", "Generated patch operation requires a non-empty find value.");
    }
    if (!operation.replaceWith.trim()) {
        return fail("MISSING_REQUIRED_FIELDS", "Generated patch operation requires a non-empty replaceWith value.");
    }
...
//# sourceMappingURL=canConvertGeneratedPlanToPatchPlan.js.map