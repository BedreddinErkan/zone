...
// Shared helpers
// ---------------------------------------------------------------------------
function issue(code, severity, message, filePath) {
...
// Test Engineer rules
// ---------------------------------------------------------------------------
function validateTestEngineerOutput(patches) {
    const issues = [];
    for (const patch of patches) {
        ...
        if (!hasExpect && !hasCyAssert && !hasAssert) {
            issues.push(issue("MISSING_EXPECT", "error", "Test file contains no expect(), cy.should(), or assert — output is not a valid test.", fp));
        }
        ...
        if (isAsync && !hasAwait) {
            issues.push(issue("MISSING_AWAIT", "error", "Async test function found but no await keyword — likely missing async calls.", fp));
        }
        ...
        for (const p of placeholderSelectors) {
            if (hasPattern(c, p)) {
                issues.push(issue("PLACEHOLDER_SELECTOR", "error", "Test contains placeholder selector — not safe to apply as-is.", fp));
                break;
            }
        }
        ...
        if (hasPattern(c, /https?:\/\/(localhost|127\.0\.0\.1|example\.com|your-app)/i)) {
            issues.push(issue("HARDCODED_URL", "warning", "Test contains hardcoded URL — consider using base URL config or env variable.", fp));
        }
        // EMPTY_TEST_BODY: it/test block with empty body
        if (hasPattern(c, /\b(it|test)\s*\([^)]+,\s*(?:async\s*)?\(\s*\)\s*=>\s*\{\s*\}/)) {
            issues.push(issue("EMPTY_TEST_BODY", "error", "Empty test body detected — test will pass trivially and provides no coverage.", fp));
        }
        ...
        if (testBlockCount > 0 && expectCount > 0 && expectCount < testBlockCount) {
            issues.push(issue("LOW_ASSERTION_RATIO", "warning", `Found ${testBlockCount} test block(s) but only ${expectCount} expect call(s) — some tests may lack assertions.`, fp));
        }
    }
    return issues;
}
...
// Developer rules
// ---------------------------------------------------------------------------
function validateDeveloperOutput(patches) {
    const issues = [];
    for (const patch of patches) {
        const c = patch.content;
        const fp = patch.filePath;
        ...
        for (const p of templateMarkers) {
            if (hasPattern(c, p)) {
                issues.push(issue("GENERIC_TEMPLATE", "warning", "Output contains template/placeholder markers — review before applying.", fp));
                break;
            }
        }
        ...
        if (isUiFile) {
            ...
            if (isShell) {
                issues.push(issue("FULL_UI_OVERWRITE", "error", "UI file appears to be a minimal shell (< 15 lines, no imports) — likely a full overwrite with generic template.", fp));
            }
        }
        ...
        if (unicodeMatches.length > 10 || unicodeDensity > 2) {
            issues.push(issue("UNICODE_ESCAPE_DETECTED", "error", `Output contains ${unicodeMatches.length} unicode escape sequences (density: ${unicodeDensity.toFixed(1)}/line) — LLM likely hallucinated encoded content instead of real code.`, fp));
        }
        ...
        if ((usesUseState || usesUseEffect) && !hasReactImport) {
            issues.push(issue("MISSING_REACT_IMPORT", "error", "File uses React hooks (useState/useEffect) but is missing React import.", fp));
        }
        ...
        if (isModuleFile && !hasPattern(c, /\bexport\b/)) {
            issues.push(issue("NO_EXPORT", "warning", "File has no export statement — may be incomplete or truncated.", fp));
        }
    }
    return issues;
}
...
// Data Analyst rules
// ---------------------------------------------------------------------------
function validateDataAnalystOutput(patches) {
    const issues = [];
    for (const patch of patches) {
        const c = patch.content;
        const fp = patch.filePath;
        ...
        if (!isSqlFile && !hasSqlContent)
            continue;
        // UNSAFE_DROP: DROP TABLE without IF EXISTS guard
        if (hasPattern(c, /DROP\s+TABLE\s+(?!IF\s+EXISTS)/i)) {
            issues.push(issue("UNSAFE_DROP", "error", "DROP TABLE without IF EXISTS — destructive operation that may fail on missing table.", fp));
        }
        // TRUNCATE_DETECTED: always block
        if (hasPattern(c, /\bTRUNCATE\b/i)) {
            issues.push(issue("TRUNCATE_DETECTED", "error", "TRUNCATE detected — irreversible bulk delete. Manual review required.", fp));
        }
        ...
        if (hasPattern(c, /DELETE\s+FROM\s+\w+\s*(?:;|$)/im) &&
            ...
            issues.push(issue("UNBOUNDED_DELETE", "error", "DELETE statement without WHERE clause — will delete ALL rows in the table.", fp));
        }
        ...
        if (hasPattern(c, /UPDATE\s+\w+\s+SET\b/i) &&
            ...
            issues.push(issue("UNBOUNDED_UPDATE", "error", "UPDATE statement without WHERE clause — will update ALL rows in the table.", fp));
        }
        ...
        if (isMigration && !hasPattern(c, /\b(rollback|down|revert)\b/i)) {
            issues.push(issue("MISSING_ROLLBACK", "warning", "Migration file appears to have no rollback/down section.", fp));
        }
    }
    return issues;
}
...
// Verdict computation
// ---------------------------------------------------------------------------
function computeVerdict(issues) {
 ...
 */
function validateLlmOutput(role, patches) {
    let issues = [];
    switch (role) {
        case "test_engineer":
...
//# sourceMappingURL=validateLlmOutput.js.map