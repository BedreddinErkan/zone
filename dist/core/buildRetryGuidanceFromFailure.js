...
function extractActual(message) {
    return firstMatch(message, [
        /Received(?:\s+string)?:\s*["'`]?([^"'`\n]+)["'`]?/i,
        /Received:\s*([^\n]+)/i,
        /but got\s+["'`]([^"'`\n]+)["'`]/i,
...
function extractIncorrectAssumption(message, reason, expected, actual) {
    if (expected && actual) {
...
function buildRootCause(reason, expected, actual) {
...
function buildRealBehavior(actual, reason) {
    if (actual)
        return `Observed behavior/output: "${actual}".`;
    if (reason === "verification_timeout")
        return "Observed behavior: the expected condition was not reached before timeout.";
    if (reason === "unsupported_behavior_assumption")
        return "Observed behavior: the requested selector, API, or behavior was not found or was unstable.";
    if (reason === "wrong_repo_assumption")
        return "Observed behavior: verification could not find the assumed repo file, path, script, or structure.";
    return "Observed behavior: verification rejected the previous generated output.";
}
function buildRequiredFix(reason, expected, actual) {
...
function buildFixStrategy(reason) {
    if (reason === "assertion_mismatch") {
...
function buildConstraint(message, reason, repeatedFailure) {
...
function buildScopeConstraint(reason) {
    if (reason === "assertion_mismatch") {
...
function analyzeFailure(input) {
    ...
    const incorrectAssumption = extractIncorrectAssumption(message, normalizedFailureReason, expected, actual);
...
function buildRetryGuidanceFromFailure(input) {
    ...
    const strictness = (input.repeatedFailureCount ?? 0) >= 2
        ? "escalated"
        : repeatedFailure
            ? "strict"
            : "normal";
    const requiredFix = buildRequiredFix(analysis.normalizedFailureReason, analysis.expected, analysis.actual);
    const constraint = buildConstraint(message, analysis.normalizedFailureReason, repeatedFailure);
    const escalation = strictness === "escalated"
    ...
    return {
        ...
        minimalPatchDiscipline: "Prefer the smallest targeted patch. For tests, fix assertions/fixtures before adding new product logic unless the task explicitly requires product behavior changes.",
