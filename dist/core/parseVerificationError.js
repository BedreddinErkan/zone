...
exports.parseVerificationError = parseVerificationError;
exports.parseVerificationErrorWithRepo = parseVerificationErrorWithRepo;
function normPath(p) {
    return String(p || "").replace(/\\/g, "/").replace(/^\s+|\s+$/g, "");
}
function looksLikeStructuralOrTooling(textLower) {
    return (textLower.includes("no tests found") ||
...
function extractAllFileMentions(summary) {
...
function pickFailingFileAndLine(summary) {
    const s = String(summary || "").replace(/\r\n/g, "\n");
    ...
    const fileReGlobal = /([A-Za-z]:[\\/][^\s:()]+?\.(?:ts|tsx|js|jsx)|(?:\.{0,2}\/)?(?:[\w.-]+\/)+[\w.-]+\.(?:ts|tsx|js|jsx))\:(\d+)(?:\:(\d+))?/g;
    ...
    if (firstUser)
        return { file: firstUser.file, line: firstUser.line };
...
function classifyErrorType(textLower) {
    if (textLower.includes("no tests found") ||
        textLower.includes("no test files") ||
        textLower.includes("could not find tests")) {
    ...
    if (textLower.includes("cannot find module") ||
...
function parseVerificationErrorImpl(failureSummary, patchedFiles, repoPath) {
    ...
    const firstRepoLocalAffected = firstAffected && isInsideRepo(firstAffected) ? firstAffected : null;
    // Otherwise, pick the first repo-local file mentioned anywhere in affectedFiles.
    const anyRepoLocalAffected = affectedNorm.find((p) => p && isInsideRepo(p)) ?? null;
    // Next best: any non-node_modules file (relative paths etc.)
    const firstUserFileFromAffected = affectedNorm.find((p) => p && !isNodeModulesPath(p)) ?? null;
    // Prefer repo-local affected file over stack when stack is in node_modules OR absent.
    const preferredAffected = firstRepoLocalAffected ?? anyRepoLocalAffected ?? firstUserFileFromAffected;
    const failingFile = failingFromStack == null
        ? preferredAffected
        : isNodeModulesPath(failingFromStack)
            ? preferredAffected
            : failingFromStack;
    const isToolingIssue = errorType === "no_tests_found" || looksLikeStructuralOrTooling(lower);
    const failingFileIsPatched = failingFile != null &&
    ...
    const isPreExisting = (errorType === "no_tests_found" && failingFile == null) ||
...
function parseVerificationError(failureSummary, patchedFiles) {
    return parseVerificationErrorImpl(failureSummary, patchedFiles);
}
function parseVerificationErrorWithRepo(failureSummary, patchedFiles, repoPath) {
    return parseVerificationErrorImpl(failureSummary, patchedFiles, repoPath);
}
//# sourceMappingURL=parseVerificationError.js.map