...
function isSmallTask(task) {
    return /\b(small|tiny|minor|simple|minimal|quick|spacing|copy|label|text|color|rename|typo|badge)\b/i.test(task);
}
function repoHasTestFiles(repoFiles) {
    return repoFiles.some((filePath) => /(^|\/)(test|tests|__tests__)\//i.test(normalizePath(filePath)) ||
...
function looksRelatedToTask(filePath, task) {
...
function hasSyntaxRedFlags(patchContent) {
    return (/\b(?:const|let|var)\s+\w+\s*=\s*undefined\b/.test(patchContent) ||
...
function verifyPatch(input) {
    ...
    const unrelatedNewFiles = changedFiles.filter((filePath) => !normalizedRepoFiles.has(normalizePath(filePath)) &&
...
//# sourceMappingURL=verifyPatch.js.map