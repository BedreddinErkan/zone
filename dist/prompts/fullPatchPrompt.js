...
function buildFullPatchPrompt(input) {
    const { task, filePath, fileContent, repoSummary, relatedContext, plannerSuggestedFile, taskIntent, normalizedTaskIntent, outputMode = "full_content", executionPlanContext, } = input;
    const plannerTargetInstruction = plannerSuggestedFile && plannerSuggestedFile.trim()
        ? `PLANNER TARGET
