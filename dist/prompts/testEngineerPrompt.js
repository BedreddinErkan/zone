...
function buildTestEngineerPrompt(input) {
    ...
    const { complexity, hints, suggestedPatterns } = (0, detectTestComplexity_js_1.detectTestComplexity)(input.task);
    const pageObjectSection = pageObjectContents.length > 0
        ? pageObjectContents
            .map((f) => `FILE: ${f.path}\n\`\`\`\n${f.content}\n\`\`\``)
            .join("\n\n")
        : "(no page objects found)";
    const pageObjectHints = pageObjectContents.length > 0
        ? pageObjectContents.map((f) => `- ${f.path}`).join("\n")
        : "(no page object usage hints found)";
    const stepDefinitionSection = stepDefinitionContents.length > 0
        ? stepDefinitionContents
            .map((f) => `FILE: ${f.path}\n\`\`\`java\n${f.content}\n\`\`\``)
            .join("\n\n")
        : "(no existing step definition examples found)";
    const featureSection = featureContents.length > 0
        ? featureContents
            .map((f) => `FILE: ${f.path}\n\`\`\`gherkin\n${f.content}\n\`\`\``)
            .join("\n\n")
        : "(no existing feature file examples found)";
    const existingTestSection = existingTestContents.length > 0
    ...
    const outputSection = framework.framework === "cucumber_java"
