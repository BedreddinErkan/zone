...
exports.validatePatchCorrectness = validatePatchCorrectness;
...
function stripCommentsForComparison(input) {
...
function normalizeForSanityCompare(input) {
    return normalizeWhitespace(stripCommentsForComparison(input));
}
function fastDiffCounts(before, after) {
...
function estimateChangedLineRange(before, after) {
...
function getImportRegionEndLine(original) {
...
function isSkippedLayer1File(filePath) {
    const p = filePath.toLowerCase();
...
function minimalBraceBracketBalance(layer, content) {
    let braces = 0;
    let brackets = 0;
...
function analyzeDelimiterCounts(content) {
...
function computeDelimiterDelta(before, after) {
    return {
        braces: {
...
function delimiterWorsened(before, after) {
    const worsenedKind = (b, a) => a.opens > b.opens || a.closes < b.closes;
    return (worsenedKind(before.braces, after.braces) ||
        worsenedKind(before.brackets, after.brackets) ||
        worsenedKind(before.parens, after.parens));
}
function delimiterUnchangedOrImproved(before, after) {
    const okKind = (b, a) => (a.opens === b.opens && a.closes === b.closes) || (a.opens <= b.opens && a.closes >= b.closes);
    return (okKind(before.braces, after.braces) &&
        okKind(before.brackets, after.brackets) &&
        okKind(before.parens, after.parens));
}
function getLineColumn(src, index) {
...
function snippetAround(src, index, radius = 40) {
    const i = Math.max(0, Math.min(index, src.length));
...
function formatDelimiterDiagnostic(input) {
    const parts = [];
    parts.push(`Unbalanced delimiters detected: ${input.subtype} at line ${input.line}, column ${input.column} (index ${input.index}).`);
    if (input.expected || input.actual) {
        parts.push(`Expected ${input.expected ?? "?"}, got ${input.actual ?? "?"}.`);
    }
    if (input.char)
        parts.push(`Char: ${JSON.stringify(input.char)}.`);
    if (input.stackTop)
        parts.push(`Stack top: ${JSON.stringify(input.stackTop)}.`);
    parts.push(`Snippet: ${input.snippet}`);
    return parts.join(" ");
}
function scanDelimitersWithDiagnostics(src) {
    const s = src.replace(/\r\n/g, "\n");
    const stack = [];
    const strippedForJsxChars = [];
    const writeStripped = (c) => strippedForJsxChars.push(c);
    ...
    function isJsxClosingTag(srcText, idx) {
        if (srcText[idx] !== "/")
            return false;
        let j = idx - 1;
        while (j >= 0 && /\s/.test(srcText[j] ?? ""))
            j -= 1;
        if (j < 0 || srcText[j] !== "<")
            return false;
    ...
    function isInsideJsxTag(srcText, idx) {
    ...
    let jsxExprDepth = 0;
    let jsxReturnMode = "code";
    const events = [];
    const pushEvent = (index, char, action) => {
    ...
    for (let i = 0; i < s.length; i += 1) {
        ...
        if (mode === "code" && ch === "<") {
            ...
            const allowedPrev = lastWord === "return" ||
                lastNonWsChar === "" ||
                lastNonWsChar === "\n" ||
                lastNonWsChar === "(" ||
                lastNonWsChar === "{" ||
...
function layer0DiffSanity(input) {
    ...
    if (input.originalContent.trim().length > 0 &&
...
function layer1LexicalIntegrity(input) {
    ...
    const deltaBalanced = deltaCounts.braces.opens === deltaCounts.braces.closes &&
    ...
    const localizedValidationMode = diffCounts.totalChangedLines <= 5 && additiveOnly && deltaBalanced;
    if (localizedValidationMode) {
        console.log("[zone-validator-localized-mode]", JSON.stringify({ enabled: true, reason: "small_safe_patch" }));
    }
    ...
    if (!scan.ok) {
        ...
        console.log("[zone-validator-delta]", JSON.stringify({
        ...
        if (localizedValidationMode) {
            ...
            if (scan.lexicalScanIncomplete) {
                issues.push(issue({
                    ...
                    severity: "warn",
                    message: "Lexical scan could not confidently parse a regex literal; results may be incomplete.",
                }));
            }
            return { issues, strippedForJsx: input.updatedContent };
        }
        console.log("[zone-delimiter-trace]", JSON.stringify({ filePath: input.filePath, events: scan.events }));
        ...
        console.log("[zone-delimiter-diagnostic]", JSON.stringify({
            filePath: input.filePath,
...
const BROKEN_CALL_AT_EOL = /\b([a-zA-Z_$][\w$]*)\s*\(\s*(?:["'`][^"'`\n]*)?$/m;
const INLINE_KEYWORD_IN_CALL = /\b(?:[a-z_$][\w$]*)\s*\(\s*["'`][^"'`\n]*["'`]?\s+(if|return|for|while|switch)\b/;
...
function layer2LanguageHeuristics(input) {
    ...
    if (brokenCall) {
        ...
        if (reserved.has(name)) {
            ...
            issues.push(issue({
                layer: 2,
                code: name === "setSuccess" || name === "setError" || name === "setFormError"
                    ? "broken_setter_call"
                    : "broken_call_at_eol",
                severity: "block",
...
function jsxTagBalanceWarn(stripped, diffAddedLines) {
    if (diffAddedLines <= 0)
        return [];
    ...
    for (const m of s.matchAll(/<([A-Za-z][\w:-]*)\b[^>]*\/>/g)) {
        if (m.index != null)
            selfClosing.add(m.index);
    }
    for (const m of s.matchAll(/<\/([A-Za-z][\w:-]*)\s*>/g)) {
        closeCount += 1;
    }
    for (const m of s.matchAll(/<([A-Za-z][\w:-]*)\b[^>]*?>/g)) {
        if (m.index != null && selfClosing.has(m.index))
            continue;
    ...
    const imbalance = openCount - closeCount + (fragmentsOpen - fragmentsClose);
    if (imbalance !== 0) {
        return [
            issue({
                layer: 2,
...
function collectExportedSymbols(src) {
    const names = new Set();
...
function layer3StructuralPreservation(input) {
...
function countMatches(haystack, pattern) {
    let count = 0;
    for (const _m of haystack.matchAll(pattern))
        count += 1;
    return count;
}
function containsSubmitHandlerSymbol(src) {
    return /\b(handleSubmit|onSubmit)\b/.test(src);
}
function layer4TaskScopePreservation(input) {
    const t = input.taskConstraints;
    const shouldRun = input.isConstrained ||
    ...
    if (t.avoidsNewApiCall) {
        const beforeApi = countMatches(input.originalContent, /\bfetch\s*\(/g) +
            countMatches(input.originalContent, /\baxios\./g) +
            countMatches(input.originalContent, /\bapi\.(get|post|put|patch|delete)\b/g);
        const afterApi = countMatches(input.updatedContent, /\bfetch\s*\(/g) +
            countMatches(input.updatedContent, /\baxios\./g) +
            countMatches(input.updatedContent, /\bapi\.(get|post|put|patch|delete)\b/g);
...
function validatePatchCorrectness(input) {
    const blocking = [];
    const warnings = [];
    let layersRun = 0;
    let shortCircuitedAt = null;
    ...
    if (hasBrokenImport && !touchesImport) {
        ...
        for (let i = blocking.length - 1; i >= 0; i -= 1) {
            if (blocking[i]?.code === "broken_import_line")
                blocking.splice(i, 1);
        }
        warnings.push(...ignored.map((i) => ({
            ...i,
            severity: "warn",
            message: i.message + " (ignored: patch did not touch import region)",
        })));
        console.log("[zone-validator-override]", JSON.stringify({
    ...
    if (input.strictMode &&
        ...
        blocking.length === 0 &&
        warnings.length > 0) {
        const promoted = warnings.map((w) => ({ ...w, severity: "block" }));
...
//# sourceMappingURL=patchCorrectnessValidator.js.map