...
with it. Checked directly against the current built prompt, not swept exhaustively at the time: `SEARCH
FIRST`, a separate, always-included block unrelated to this preamble, unconditionally states "use
search_in_files BEFORE read_file" — and `search_in_files` is absent from this same archetype's own offered
set, for the identical structural reason `run_command` was. `search_in_files` was the case that surfaced while
checking, not the whole of what a full sweep afterward found: item 100 records the class, and
`search_in_files` is not its largest instance.
...
investigation assembler's tool-list entry); gated on archetype or plan shape, correlated with the offered set
but not identical to it — the Q&A and investigation preambles themselves, `ANSWER_SUMMARY` versus
`buildPatchSummary`, the read-only-archetype branch generally; and unconditional, with no relationship to
tools at all — the plan-progress block (`TodoWrite`), `TASK SUBAGENTS`, `SEARCH FIRST`, `DIVERGENCE CHECK`,
and several others. The defect this item catalogues lives in the third group: text that instructs a specific
tool by name, written once, included regardless of which configuration renders it.

**Not every mention is a defect, the same distinction item 98's own second fix turned on.** An instruction to
use an absent tool is the defect; a prohibition on an absent tool is noise (the notice already says so); an
incidental mention describing a result format or payload is neither. `run_command` appears in most of the
prompt's unconditional blocks, but after item 97's fix every one of those appearances is incidental
(`INTERPRETING COMMAND OUTPUT`) or a prohibition (`USER EDIT REJECTION`) — none is an instruction, so
`run_command` contributes nothing to this item's own count.

**Nine of nineteen configurations tested carry at least one instruction-grade reference to a tool they don't
offer**, resolved through the real registry, generated by the real assemblers: `simple_add`, `question`,
`investigation` (the archetype), `tier:simple`, `tier:medium`, `subagent:worker`, `subagent:explore`,
`subagent:verifier`, and an approved answer-only plan. The other ten — `targeted_fix`, `refactor`, `debug`,
`complex_multi_file`, `tier:complex`, the chat default, the patch default, plan-investigation, scope-audit, and
the investigation-mode default — carry none; where `TodoWrite` or `run_command` appear in that second group,
it is only as the investigation assembler's own prohibition ("Do not call…"), not an instruction. Counted by
tool, across the nine: `Task` in all nine, the only one present in every defective configuration; `TodoWrite`
and `revert_patch` tied second at seven each; `multi_edit` at five; `apply_patch` and `write_file` tied at
four; `search_in_files` — the case that surfaced this item, checking item 98's own closing question — at
three; `find_references` at two, the smallest. `search_in_files` is not the largest instance and not the
second-largest; it is seventh of eight by this count.

**Every defective cell is the same shape: the notice names the tool.** Checked against the real
`buildToolAbsenceBlock` for all nine — in each, every instructed-but-absent tool appears in the withheld list.
...
assemblers are invoked from that same expression; threading it the way item 97's own fix threaded
`investigationCommandTool` requires nothing new to be derived, for any of the nine. The chat assembler needs no
fix: its tool set is fixed by construction, so its own references cannot go stale against it.

**What this sweep covered, and named rather than assumed complete.** Nineteen configurations, three
assemblers. Not covered: `planAnnotationsBlock`, which names `Task` once and renders only when an execution
plan carries subagent-eligible steps, so the `Task` count above may understate plan-carrying runs specifically
— passed empty here, not enumerated. `projectMemoryBlock` is user content, not instruction.
`importContextSummary` names `read_file`, offered everywhere checked. `TRUST_PHASE1_DIRECTIVE` carries no tool
names. Subagent reachability (`verifier`, item 92) was taken as given, not re-derived.

**The sweep that produced this item made three kinds of mistake against itself, two in its own construction
and one in its own arithmetic — all found and corrected before landing here, none silently absorbed.** Passing
the plan-progress block as an empty string, before its inclusion condition was confirmed, suppressed the
single largest defect in the whole matrix — `TodoWrite` was invisible until the real block replaced the
placeholder. Routing the chat default through the wrong assembler produced eight spurious absences that a
correctly-targeted, standalone check resolved to zero. Both are errors in what the measurement's own setup fed
it, not in how a correct result was read. A third, different kind, found only while transcribing this item:
the establish pass's own prose summary — nineteen configurations reported as eighteen, nine defective reported
as ten, `TodoWrite`'s seven configurations reported as eight — did not match its own detailed table, which was
correct throughout. Re-counted twice here, independently, against that table: by hand, and by a second,
separate mechanical pass. Both agree with the table, not the prose. The figures in this item are the
re-counted ones.

**Essay candidate — the first two mistakes above, checked against the twenty-two by mechanism, not the
third.** A measurement's own setup substituted an unconfirmed placeholder for part of the artifact it meant to
measure, and the substitution was invisible in the result except where the placeholder happened to be exactly
the part a check was designed to catch. Closest by theme is the twenty-first (an attribution about a measured
subject, unverified against the instrument that produced it) — but that pattern is post-hoc: a valid
measurement already ran to completion, and the mistake was in explaining its result afterward. This is
pre-hoc: no result existed yet when the placeholder was chosen, and the mistake is in what the measurement was
given to measure, not in how its output was read. Also checked against the fourth (a default on an injected
seam is a silent fallback) — that pattern's own defect lives in production code, a call site forgetting to
pass a real value into an optional parameter a function itself defaults; here there is no default in the
function's own signature, only a placeholder the investigator's own new script chose before confirming it was
safe to choose. **Novel. Accepted**, on the same category basis as the nineteenth through twenty-second: a
fact about how this arc built its own measurement, not about the subject being measured. Recorded as the
twenty-third pattern. The third mistake — the summary not matching its own table — is recorded in this item's
own text, not carried into the essay: it is a transcription error against already-correct data, not a
...
across `assembleAgentSystemPrompt` and `assembleInvestigationSystemPrompt`, both in `agentLoop.ts`. See item 97
...
## A twenty-third pattern: a measurement's own setup can silently substitute a placeholder for the part of the artefact it means to measure

A sweep built to find every prompt instruction naming an unavailable tool constructed its own test inputs by
hand, one field at a time. Two fields were passed as empty strings before their own inclusion condition had
been checked — not because they were confirmed conditional, but because their real content hadn't been read
yet. One of those two turned out to be unconditional, always included, and it carried the single largest
defect the sweep was built to find. The sweep's first result reported that defect as absent. It was never
absent from the real prompt; it was absent from the placeholder standing in for it.

This is a different mistake from reading too broad a region of a correct artefact. There, the thing being
checked is real and whole, and the check's own scope is what fails. Here, no real artefact was checked at all
for the affected field — a stand-in was, chosen because building the real one hadn't been done yet, and the
stand-in's own emptiness silently redefined what "clean" meant for that field. A measurement that substitutes
a placeholder for an unconfirmed part isn't skipping that part; it's measuring a different, non-existent
configuration instead, and nothing about a clean result distinguishes the two.

The rule: before trusting a sweep's own clean result for any part of a composite artefact, name what stood in
for each part that wasn't the real, produced value — and confirm each stand-in was checked equivalent to what
