export function buildFeaturePlanningPrompt(input: {
  task: string;
...
export function buildPatchPlanningPrompt(input: {
  task: string;
  projectSummary: string;
  projectNotes: string[];
  ...
  const suggestedFilesText =
    input.suggestedFiles.length > 0
      ? input.suggestedFiles
          .map(
            (file) =>
              `- ${file.path} | action=${file.action} | reason=${file.reason}`
          )
          .join("\n")
      : "- No suggested files";

  const fileContextsText =
    input.fileContexts.length > 0
      ? input.fileContexts
          .map(
            (file) => `
FILE: ${file.path}
