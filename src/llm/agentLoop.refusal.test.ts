...
describe("Fable hint appended on content_filter with Fable model", () => {
  beforeEach(() => {
    vi.stubEnv("ZONE_ANTHROPIC_MODEL_HIGH", "claude-fable-5");
  ...
  it("result.refusal contains Opus hint text when model is claude-fable-5", async () => {
    installScript(mocks.createChatCompletion, [scriptRefusal()]);

    const result = await runAgentLoop({
      task: "generate exploit code",
      repoPath,
      runId: "test-refusal-fable",
    });

    expect(result.terminationReason).toBe("natural_completion");
    expect(result.refusal).toContain("claude-opus-4-8");
  });

  it("Fable hint costUsd reflects claude-fable-5 rates (input=$10, output=$50)", async () => {
    installScript(mocks.createChatCompletion, [scriptRefusal()]);

    const result = await runAgentLoop({
      task: "generate exploit code",
      repoPath,
      runId: "test-refusal-fable-cost",
    });

    // claude-fable-5 rates: input=$10/Mtok, output=$50/Mtok
    const expected = (10 / 1_000_000) * 10 + (5 / 1_000_000) * 50;
    expect(result.costUsd).toBeCloseTo(expected, 7);
  });
});
