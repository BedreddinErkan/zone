...
/**
 * apply_patch's own FIND/REPLACE segmentation walk (ledger item 16). Forward index scan:
 * find the next FIND marker, the next REPLACE marker after it, the next FIND marker after
 * that (or end of string) closes the block. Normalizes smart quotes on each block's find/
 * replace as they're extracted — this is the ONE normalization step inside segmentation
 * itself; CRLF/bare-CR normalization happens later, per matched block, at match time
 * (normalizeEol above), not here (ledger item 18).
 *
 * KNOWN DEFECT, NOT FIXED HERE: this walk uses the same substring-anywhere marker
 * matching as the imbalance check in apply_patch's handler, so an embedded, own-line,
 * MATCHED FIND/REPLACE pair inside what should be one block's REPLACE content (e.g. a doc
 ...
 * example text. The imbalance check cannot catch this shape: a matched embedded pair
 * raises both findMarkerCount and replaceMarkerCount together, so they stay equal, the
 * rejection branch never fires, and no [zone-apply-patch-marker-imbalance] record of this
 * can exist. Silent wrong-content write, not a rejection — worse than the false positive
 * that check addresses. Deliberately unfixed pending its own pass: fixing it changes which
 * patches get *accepted*, not just which get rejected more legibly (ledger item 2).
 */
export function segmentApplyPatchBlocks(patch: string): {
...
export async function executeTool(
  ...
  try {
    ...
    if (toolName === "apply_patch") {
      ...
      // Segmentation walk extracted to segmentApplyPatchBlocks (ledger item 16) — see its
      // doc comment for the KNOWN DEFECT this walk carries forward unchanged.
      const { blocks, sqFindTotal, sqReplaceTotal } = segmentApplyPatchBlocks(patch);
