...
function isSkippableMethodName(name: string): boolean {
...
function indexOfUnescapedBraceOpen(line: string, fromIndex = 0): number {
...
function extractNameFromArrowParams(raw: string): { paramsRaw: string } {
  const trimmed = raw.trim();
  if (!trimmed) return { paramsRaw: "" };
  ...
  try {
    ...
    for (let li = 0; li < lines.length; li += 1) {
      ...
      if (methodMatch) {
        ...
        if (isSkippableMethodName(name)) continue;
    ...
    return {
      ok: false,
      functions: [],
      error: `[extractFunctionRanges] ${filePath}: ${
