...
export type ResourceScope =
  ...
  | "collection"
  | "unknown";
export type ResourceKind = "nested_resource" | "parent_resource" | "unknown";
...
function detectAction(task: string): CrudAction {
  if (includesAny(task, DELETE_WORDS)) return "delete";
  if (includesAny(task, UPDATE_WORDS)) return "update";
  if (includesAny(task, CREATE_WORDS)) return "create";
  if (includesAny(task, READ_WORDS)) return "read";
  return "unknown";
}

function detectParentResource(task: string): string | undefined {
...
export function parseTaskIntent(rawTask: string): TaskIntent {
  const normalizedTask = normalizeTask(rawTask);
  const warnings: string[] = [];

  const action = detectAction(normalizedTask);

  const matchedNestedConfig = KNOWN_NESTED_MAP.find((item) =>
    item.match.test(normalizedTask)
  );

  const mentionsNestedItem =
    !!matchedNestedConfig || includesAny(normalizedTask, NESTED_ITEM_WORDS);

  const detectedParentResource =
    matchedNestedConfig?.parent ?? detectParentResource(normalizedTask);

  const detectedNestedResource = matchedNestedConfig?.nested;

  const resourceKind: ResourceKind = mentionsNestedItem
    ? "nested_resource"
  ...
  if (action === "unknown") {
    warnings.push(
      "Could not confidently determine CRUD action from task wording."
    );
  }

  if (!detectedParentResource && resourceKind !== "unknown") {
    warnings.push(
      "Resource kind was inferred, but parent resource could not be confidently identified."
    );
  }
  ...
  if (resourceKind === "nested_resource") {
    warnings.push(
      "Nested resource detected. Parent resource should not be directly replaced."
    );
  }

  if (action === "update" && resourceKind === "nested_resource") {
    warnings.push(
      "Update should target the nested item only, not the whole parent record."
    );
  }

  if (scope === "collection" && action === "delete") {
    warnings.push(
      "Collection-level delete detected. Confirm that bulk deletion is intended."
    );
  }
