...
function normalizeTrailingComma(line: string): string {
...
function deriveSingleLineEdit(find: string, replace: string): { findLine: string; replaceLine: string } | null {
  const a = normalizeNewlines(find).split("\n");
  const b = normalizeNewlines(replace).split("\n");
  const min = Math.min(a.length, b.length);
  const diffs: Array<{ a: string; b: string }> = [];
  for (let i = 0; i < min; i += 1) {
    const aa = (a[i] ?? "").trimEnd();
    const bb = (b[i] ?? "").trimEnd();
    if (aa !== bb) diffs.push({ a: aa, b: bb });
  }
...
function findBestFuzzyWindowMatch(input: {
  original: string;
  find: string;
  anchorLineRange?: { start: number; end: number };
}): { ok: true; startLine: number; lineCount: number; score: number } | { ok: false; reason: string } {
  ...
  const endAt =
    input.anchorLineRange?.end ?? Math.max(0, originalLinesNorm.length - 1);

  const windowLen = Math.min(findLinesNorm.length, 12);
  const scan = (requireSecondLine: boolean): Array<{ startLine: number; score: number }> => {
    const candidates: Array<{ startLine: number; score: number }> = [];
    for (let i = startAt; i <= endAt; i += 1) {
      if ((originalLinesNorm[i] ?? "") !== first) continue;
  ...
  if (best.score < 0.8) return { ok: false, reason: `low_score_${best.score.toFixed(2)}` };
...
export function tryRecoverDeveloperPatchFromModelOutput(input: {
  ...
  if (hits !== 1) {
    ...
    const minimalPatchMode =
      taskNorm.includes("remove stray character") && patchChangedLines <= 1;

    if (minimalPatchMode) {
      const findLines = normalizeNewlines(find).split("\n").map((l) => l.trimEnd());
      const replaceLines = new Set(
        normalizeNewlines(replace).split("\n").map((l) => l.trimEnd())
      );
    ...
    if (countOccurrences(original, resolvedFind) !== 1) {
      ...
      if (fuzzy.ok) {
        ...
        const fnMatch = find.match(
          /\b(?:export\s+default\s+function|function)\s+([A-Za-z0-9_]+)\b/
        );
        const fnName = fnMatch?.[1];
        if (fnName) {
          const originalLinesRaw = normalizeNewlines(original).split("\n");
          const originalLinesNorm = originalLinesRaw.map((l) =>
            normalizeLineForMatch(l)
          );
