...
// file's plan_full_patch tests never supply preGeneratedPlan, so plannerStep and
// generateExecutionPlan both reach createLLMClient(); each call site already wraps the whole
// call in try/catch, so a synchronous throw here degrades gracefully with no assertion changes.
