import type { GeneratedPlanConversionCheck, GeneratedPlanConversionFailureCode } from "../../patch/conversion/generatedPlanConversionTypes.js";
import type { ZoneCode, ZoneStageResultV2 } from "../output/zoneStageTypes.js";
import { buildStageResultV2 } from "./buildStageResultV2.js";

function resolveFailureSummary(code: GeneratedPlanConversionFailureCode): string {
  switch (code) {
...
function resolveFailureCode(code: GeneratedPlanConversionFailureCode): ZoneCode {
  switch (code) {
    case "PLACEHOLDER_FILE_PATH":
    case "PLACEHOLDER_TEXT":
...
export function buildConversionStageResult(
  check: GeneratedPlanConversionCheck
): ZoneStageResultV2 {
  if (check.canConvert) {
