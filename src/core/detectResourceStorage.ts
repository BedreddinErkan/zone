import type { ResourceStorageInsight } from "../types/analysis.js";
...
type DetectResourceStorageInput = {
  parentResource: string;
  nestedResource?: string;
  files: FileInput[];
  schema: SchemaSnapshot | null;
};
function toLikelyTableName(resource: string): string {
  if (!resource) return resource;
  if (resource.endsWith("s")) return resource;
  return `${resource}s`;
}
export function detectResourceStorage(input: DetectResourceStorageInput): ResourceStorageInsight {
...
const parentTable = toLikelyTableName(parentResource);
    ...
    const jsonLikeColumn = schema.columns.find(
      (col) =>
        col.tableName === parentTable &&
