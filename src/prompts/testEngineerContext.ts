...
function findPageObjects(files: RepoFile[]): RepoFile[] {
  return files.filter(
    (f) =>
...
function findStepDefinitions(files: RepoFile[]): RepoFile[] {
  return files.filter(
    (f) =>
...
function findFeatureFiles(files: RepoFile[]): RepoFile[] {
  return files.filter((f) => f.path.endsWith(".feature"));
}

function findConfigFiles(files: RepoFile[]): RepoFile[] {
  return files.filter(
    (f) =>
...
function findExistingTestFiles(
  ...
  switch (framework.framework) {
    ...
    case "testng":
      return files.filter(
        (f) => f.path.endsWith("Test.java") || f.path.endsWith("Tests.java")
      );
    case "pytest":
...
function buildFrameworkSummary(framework: DetectedTestFramework): string {
  const lines: string[] = [];
...
function buildPromptHints(
  framework: DetectedTestFramework,
  pageObjects: RepoFile[],
  stepDefs: RepoFile[],
  existingTests: RepoFile[]
): string[] {
  const hints: string[] = [];
  ...
  switch (framework.framework) {
    case "playwright_ts":
      ...
      if (pageObjects.length > 0) {
        hints.push(
          `Reuse existing page objects: ${pageObjects.map((f) => f.path).join(", ")}`
        );
      }
      break;
    ...
    case "cucumber_java":
      ...
      hints.push(
        "Write Java step definitions with @Given, @When, @Then annotations"
      );
      hints.push("Import io.cucumber.java.en.Given/When/Then");
      hints.push(
        `Place feature file in: ${framework.testDir ?? "src/test/resources/features/"}`
      );
      hints.push(
        "Place step definitions in: src/test/java/com/*/stepdefinitions/"
      );
      if (pageObjects.length > 0) {
        hints.push(
          `Inject existing page objects via constructor: ${pageObjects.map((f) => f.path).join(", ")}`
        );
        hints.push(
          "NEVER create new WebDriver instances — use existing page objects"
        );
      }
      if (stepDefs.length > 0) {
        hints.push(
          `Follow existing step definition patterns from: ${stepDefs.map((f) => f.path).join(", ")}`
        );
      }
      break;
  ...
  if (existingTests.length > 0) {
    hints.push(
      `Follow patterns from existing tests: ${existingTests.slice(0, 3).map((f) => f.path).join(", ")}`
    );
  }
